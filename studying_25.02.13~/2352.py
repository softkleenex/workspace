# # 최근 아시아 지역의 학생들만 참여하는 정보 올림피아드 대회가 만들어졌다. 이 대회는 온라인으로 치러지기
#  때문에 각 나라에서 이 대회에 참여하는 학생 수의 제한은 없다. 
...
gold = max(scores,  key = lambda i: i[2])
scores.remove(gold)
silver = max(scores,  key = lambda i: i[2])
if gold[0] == silver[0]:
    t = gold[0]
    scores = [i for i in scores[:] if i[0] != t]
...
copper = max(scores,  key = lambda i: i[2])
