...
req_test_num = []
for i in range(0, m):
    req_test_num.append(list(input()))


#req_test_num 서로 다른 테스트용


for i in range(0 ,m):#m개의 후보들을 검사
    #후보 i 검사 시작
    index = 0
    #print(req_test_num[i])
    for i2 in req_test_num[i]:#후보 지정
        if (index < n and remember[index] == i2):
            index = index + 1
