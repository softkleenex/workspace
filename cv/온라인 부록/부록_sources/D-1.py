...
model.compile(optimizer=tf.keras.optimizers.experimental.AdamW(learning_rate=1e-3,weight_decay=1e-4),loss=keras.losses.mean_absolute_error)
model.fit(train_dataset,epochs=50,validation_data=val_dataset,callbacks=[keras.callbacks.LambdaCallback(on_epoch_end=model.plot_images),cp_callback])
...
#         print(images.shape,noises.shape); input()
